...
int main()
{
	...
	std::cout<<"*****************   MAIN - names #5 - uzi navon"<<std::endl;
	users = db_proxy.GetUsersByAttribute(NAME, "uzi navon");
	for (const auto& user : users)
	{
		print_user(user);
	}

	std::cout<<"*****************   MAIN - names #6 - WA CA"<<std::endl;
	users = db_proxy.GetUsersByAttribute(NAME, "WA CA");
	for (const auto& user : users)
	{
		print_user(user);
	}
